...
def create_and_configure_plot(
    ...
    Каждая непрерывная ветвь рисуется отдельной линией — соединения через разрыв отсутствуют.
...
def _split_into_continuous_segments(
    ...
    так и абсолютные скачки, что особенно важно для лабораторных вариантов,
    где масштабы ветвей сильно различаются.
    """
    if len(y) <= 1:
        return [(x, y)]
