...
def get_function() -> Tuple[
    ...
    def f(x: np.ndarray) -> np.ndarray:
        return np.piecewise(
            ...
            [(x >= 0) & (x <= 1), (x > 1) & (x <= 2)],
            [
                lambda t: np.cos(t + t**3),
                lambda t: np.exp(-t**2) - t**2 + 2 * t
            ]
        )
    ...
    def f_prime(x: np.ndarray) -> np.ndarray:
        return np.where(
            (x >= 0) & (x <= 1),
            -np.sin(x + x**3) * (1 + 3 * x**2),
            -2 * x * np.exp(-x**2) - 2 * x + 2
        )
...
def run() -> None:
    ...
    print_tangent_info(1, x0, y0, slope)
    create_and_configure_plot(f, xmin, xmax, x0, y0, slope, 1)